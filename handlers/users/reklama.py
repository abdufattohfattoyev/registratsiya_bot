...
from utils.db_api.database import user_db
...
# HANDLERLAR - DEKORATORLARSIZ!

async def reklama_handler(message: types.Message, state: FSMContext = None):
    """Reklama handler - state ni tozalash bilan"""
...
async def rek_state(message: types.Message, state: FSMContext):
    ...
    if await check_admin_permission(telegram_id) or await check_super_admin_permission(telegram_id):
        ...
        if ad_type == 'ad_type_button':
            ...
            await state.update_data(ad_content=message)
            await bot.send_message(chat_id=message.chat.id, text="Reklamani yuborishni tasdiqlaysizmi?",
...
async def cancel_ad_handler(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    # Admin keyboard ni qaytarish
    from keyboards.default.keyboards import get_admin_keyboard

    await callback_query.message.edit_text("Reklama yuborish bekor qilindi 🤖❌")

    # Admin panelga qaytish xabari
    await bot.send_message(
...
def register_reklama_handlers(dp):
    """Reklama handlerlarini ro'yxatdan o'tkazish"""
    print("🔧 Reklama handlerlarini ro'yxatdan o'tkazish boshlandi...")

    try:
        # Message handlerlar - STATE BILAN
        dp.register_message_handler(reklama_handler, lambda m: m.text and m.text.strip() == "📣 Reklama", state="*")
        dp.register_message_handler(handle_time_input, state=ReklamaTuriState.time_value)
        dp.register_message_handler(rek_state, state=ReklamaTuriState.content, content_types=types.ContentType.ANY)
        dp.register_message_handler(handle_buttons_input, state=ReklamaTuriState.buttons)
        ...
        dp.register_callback_query_handler(cancel_ad_handler, lambda c: c.data == "cancel_ad", state='*')
        dp.register_callback_query_handler(confirm_ad_handler, lambda c: c.data == "confirm_ad", state='*')
        dp.register_callback_query_handler(pause_ad_handler, lambda c: c.data.startswith("pause_ad_"))
        dp.register_callback_query_handler(resume_ad_handler, lambda c: c.data.startswith("resume_ad_"))
        dp.register_callback_query_handler(stop_ad_handler, lambda c: c.data.startswith("stop_ad_"))

        print("✅ Reklama handlerlari muvaffaqiyatli ro'yxatdan o'tdi!")

    except Exception as e:
        print(f"❌ Reklama handlerlarini ro'yxatdan o'tkazishda xatolik: {e}")
        raise e